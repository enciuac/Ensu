...
const Portadas=(()=>{
  ...
  async function buscar(e){
    const q=encodeURIComponent(`${e.libro} ${e.autor.replace(/\(.*?\)/g,"")}`.trim());
    const r=await fetch(`https://openlibrary.org/search.json?q=${q}&limit=8&fields=cover_i,language`);
    if(!r.ok)throw new Error("ol");
    const j=await r.json();
    const docs=(j.docs||[]).filter(d=>d.cover_i);
    const es=docs.find(d=>(d.language||[]).includes("spa"));
    return(es||docs[0]||{}).cover_i||-1;
  }
  ...
  return{html,pedir,fallo,url,reintentar};
})();
...
const VISTAS={"":"home",inicio:"home",biblioteca:"biblioteca",cronologia:"cronologia",estadisticas:"estadisticas",tareas:"tareas",etiqueta:"etiqueta"};
function onRoute(){
  const{v,p}=parseHash();
  ...
  document.title=({home:"EnSu — Biblioteca personal",biblioteca:"Biblioteca · EnSu",cronologia:"Cronología · EnSu",estadisticas:"Estadísticas · EnSu",tareas:"Tareas · EnSu",etiqueta:`#${p} · EnSu`})[vista];
  if(cambio)window.scrollTo(0,0);
  renderVista();
}
function renderVista(){
  const f={home:renderHome,biblioteca:renderBiblioteca,cronologia:renderCronologia,estadisticas:renderEstadisticas,tareas:renderTareas,etiqueta:renderEtiqueta}[App.vista];
  if(f)f();
}
...
function renderEstadisticas(){
  ...
  const cajas=[
    [E.length,"Entradas",""],
    [term.length,"Libros leídos",`${term.filter(e=>e.fecha.startsWith(anio)).length} en ${anio}`],
...
function renderLeer(scrollTop){
  ...
  if(e.cita)body+=`<figure class="leer-cita"><span class="leer-cita-mark">“</span><blockquote class="leer-cita-txt">${esc(e.cita)}</blockquote></figure>`;
...
async function guardarProgresoRapido(id,terminar){
  ...
  const c={};
  if(terminar){c.estado="terminado";c.progreso=100;if(e.paginasTotal)c.paginaActual=e.paginasTotal;}
  else{
    ...
    if(c.progreso>=100)c.estado="terminado";
...
async function compartir(){
  ...
  const title=e?`${titulo(e)} · EnSu`:"EnSu";
  try{
...
const CAMPOS=["tipo","estado","libro","autor","fecha","paginasTotal","paginaActual","progreso","finalidad","categoria","dificultad","puntuacion","orden","tags","portada","tituloRef","reflexion","cita","vida","tension","notas"];
const fEl=k=>$("f-"+k);
...
function actualizarPreviewPortada(){
  const e={tipo:"libro",libro:fEl("libro").value||"Título",autor:fEl("autor").value,portada:fEl("portada").value.trim(),portadaId:0,id:0};
  const prev=Form.id!=null?visibles().find(x=>x.id===Form.id):null;
  if(!e.portada&&prev)e.portadaId=prev.portadaId;
...
async function guardarForm(ev){
  ...
  if(previa&&(previa.libro!==datos.libro||previa.autor!==datos.autor)&&!datos.portada){datos.portadaId=0;Portadas.reintentar(datos);}
...
const ACCIONES={
  ...
  "portada-buscar":()=>{fEl("portada").value="";const prev=Form.id!=null?visibles().find(x=>x.id===Form.id):null;if(prev){Portadas.reintentar(prev);Store.actualizarCampos(prev.id,{portadaId:0}).catch(()=>{});}actualizarPreviewPortada();toast("Se buscará la portada automáticamente.");},
